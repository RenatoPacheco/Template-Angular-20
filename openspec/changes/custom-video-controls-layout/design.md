...
3. **Tempo como `durationDisplay + timeDivider + currentTimeDisplay` (formato `total / atual`), via opcao `controlBar.children`.**
   Racional: a ordem pedida (total primeiro) nao e a default da lib, entao a ordem dos filhos e fixada explicitamente no init — o que de quebra torna o layout deterministico (decisao 2 resolvida sem fallback). Reaproveita os displays default; o `timeDivider` default ja renderiza `/`. O formato com zero-padding (`MM:SS`, `HH:MM:SS`) vem de formatador custom via API publica `videojs.setFormatTime` (o `formatTime` default nao preenche minutos com zero nem horas) — sem subclasses nem manipulacao de DOM. Efeito global: vale para todos os players video.js da pagina (hoje so existe o `app-video`); aplicado uma vez via guarda `clockFormatApplied`, sem reset no destroy para nao quebrar multiplas instancias. Detalhe: a skin esconde os 3 displays por padrao e so os revela com a classe `vjs-normalise-time-controls`, que a lib nunca adiciona sozinha — por isso o `video.scss` forca `display: flex` neles.
